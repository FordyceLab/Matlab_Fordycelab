...
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20130515-03.txt');
...
%                 uiwait(msgbox(['Set mfcs channels [' num2str(lanthanideMfcsCh) '] to ' num2str(inputPressures(2:5, ii)'.*68.94) 'mBar']));
                ...
                chip.openValves({'Mx2Out', 'Mx2In'})
                pause(0.3)
                chip.openValve('Water');
                pause(0.3)
                chip.openValve('MxWaste');
                pause(0.3)
                scr.wait(0, mixerCleanTime, [codeStr.M1 10 13 'Cleaning mixer 2'], true); 
...
%                 if ii>1
                scr.wait(mixingTime(1), mixingTime(2), [codeStr.M1 10 13 'Mixing lanthanides Mixer 2'], true);
                chip.closeValves(lanthanideValves);
                pause(0.3)
%                 chip.closeValve('MxWaste');
                chip.closeValves({'Mx2Out', 'Mx2In'});
                pause(0.3)
                ...
                scr.wait(0, dropPushStabilizationTime, [codeStr.M1 10 13 'Stabilizing Mixer 2 drop push pressure'], true);
                chip.openValve('Drops2');
                scr.wait(0, dropStabilizationTime, [codeStr.M1 10 13 'Waiting for drop stabilization Mixer 2'], true);
                if ~scr.stopSignal()
                ...
                scr.wait(polymerTime(1), polymerTime(2), [codeStr.M1 10 13 'Making Beads Mixer 2'], true);
                scope.gui.enableLiveMode(0);
