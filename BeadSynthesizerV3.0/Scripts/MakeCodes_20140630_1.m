% Script to generate 187 code set with Eu, Sm, Dy, and Tm
% Tm went bad last time.  One more attempt with 1:2 Tm, then scale back
% both code set and Tm concentration until stable.

% Also optimized the herringbone fill (1:45) and empty (~4:00) times, but won't adjust the empty with push water time. 

% P. Fordyce and B. Baxter 06/30/14
% Attempting to add additional logging about actual MFCS pressures

% Path and name of file to write log to
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140630-01.txt');
mfcsLogFile = fullfile(uigetdir(), filesep, 'PressureLog_20140630-01.txt');

% Chip serial code
chipSerial = 'BS3.0A-PMF-06.12.14-ch02';
...
% Mixer 1
mixData.M1.C1 = 1.098; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.071; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 1.202; %RTm/REu = WidthEu/WidthTm
mixData.M1.Cmix = 1/49.00; %Rmix/REu = Pmix/(PEu - Pmix)
% Mixer 2
