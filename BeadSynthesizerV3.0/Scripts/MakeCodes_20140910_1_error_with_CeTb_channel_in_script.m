% Retrying 187 codes with Tm to test Tm stability and lower LAP amount
%
% Using code Polly edited to add CeTb and remove the Push Water to the
% manifold. Updated MFCS and Ch# for each input in this file.

% B. Baxter 09/10/14

% Path and name of file to write log to
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140910-01.txt');
mfcsLogFile = fullfile(uigetdir(), filesep, 'PressureLog_20140910-01.txt');

% Chip serial code
chipSerial = 'BS3.0A-PMF-07.30.14-ch02';
...
Sm = [0, 0.0589, 0.1363, 0.2379, 0.3714, 0.5467, 0.7770];
Dy = [0, 0.0480, 0.1133, 0.2020, 0.3226, 0.4865, 0.7094];
Tm = [0, 0.0610, 0.1504, 0.2815, 0.4738, 0.7559];
CeTb = [0];
...
% [Sm, Dy, Tm, CeTb, Eu]
lanthanideMfcsCh = [6, 7, 4, 1, 5];

% Chip input valve names used for the lanthanides
% [Sm, Dy, Tm, CeTb, Eu]
lanthanideValves = {'C2'; 'C3'; 'C4'; 'C6'; 'C1'};
...
% Mixer 1
mixData.M1.C1 = 1.0000; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.025; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 1.0159; %RTm/REu = WidthEu/WidthTm
mixData.M1.C4 = 1.0; %RCeTb/REu = WidthEu/WidthCeTb
mixData.M1.Cmix = 1/19.00; %Rmix/REu = Pmix/(PEu - Pmix)
% Mixer 2
...
if ~scr.stopSignal()
    ...
    switch lower(opt)
        ...
        case 'yes'
            ...
            mfcs.setChannelsPressure([pushWaterMfcsCh, oilMfcsCh], [pushWaterPressure, oilPressure]);
