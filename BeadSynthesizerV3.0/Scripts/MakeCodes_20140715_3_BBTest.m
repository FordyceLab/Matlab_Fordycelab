% Script to generate 26 code set with Eu, Sm, Dy, and Tm using both 1:4 Dy and 1:4 Tm this
% time. 
% Retry with faster oil flow, smaller beads due to punchout mismatch at
% Bead Out

%This is my (BB) extra save with an Ro0 open at the end of polymerization
%and a close Ro0 at the end of the 5 second Flush wash after each code so
%this should flush out the bead port on this chip each round of codes...

% P. Fordyce and B. Baxter 07/15/14
% Attempting to add additional logging about actual MFCS pressures

% Path and name of file to write log to
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140715-03.txt');
mfcsLogFile = fullfile(uigetdir(), filesep, 'PressureLog_20140715-03.txt');

% Chip serial code
chipSerial = 'BS3.0B-PMF-06.12.14-ch01';
...
Sm = [0, 0.1363, 0.3714, 0.7770];
Dy = [0, 0.1133, 0.3226, 0.7094];
Tm = [0, 0.2815, 0.7559];
...
% Mixer 1
mixData.M1.C1 = 1.0597; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.0416; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 1.2373; %RTm/REu = WidthEu/WidthTm
