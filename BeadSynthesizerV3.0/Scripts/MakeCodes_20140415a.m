% Script to generate Sm/Eu + Dy/Eu codes (24 of them)
% 
%
% 
% 
...
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140415-01.txt');

% Chip serial code
chipSerial = 'BS3.0A-PMF-04.08.14-ch01';
...
Sm = [0, 0.2, 0.4, 0.6, 0.8, 1];
Dy = [0, 0.2, 0.4, 0.6, 0.8, 1];
%Tm = [0];
allLevels = {colors{1}, Sm; colors{2}, Dy}; 
%colors{3}, Tm};
...
% [Sm, Dy, Eu]
lanthanideMfcsCh = [6, 5, 7];

% Chip input valve names used for the lanthanides
% [Sm, Dy, Eu]
lanthanideValves = {'C2'; 'C3'; 'C1'};
...
% Mixer 1
mixData.M1.C1 = 1.40; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.087; %RDy/REu = WidthEu/WidthDy
%mixData.M1.C3 = 0.90; %RTm/REu = WidthEu/WidthTm
mixData.M1.Cmix = 1/99; %Rmix/REu = Pmix/(PEu - Pmix)
% Mixer 2
mixData.M2.C1 = 1.182; %RSm/REu = WidthEu/WidthSm
mixData.M2.C2 = 0.920; %RDy/REu = WidthEu/WidthDy
%mixData.M2.C3 = 1.298; %RTm/REu = WidthEu/WidthTm
mixData.M2.Cmix = 1/99; %Rmix/REu = Pmix/(PEu - Pmix)
...
mixData.M1.Resistors = {mixData.M1.R1, mixData.M1.R2, mixData.M1.R4, mixData.M1.Rmix};
% Mixer 2
...
mixData.M2.Resistors = {mixData.M2.R1, mixData.M2.R2, mixData.M2.R4, mixData.M2.Rmix};
...
if ~scr.stopSignal()
    if ~isempty(logFile)
        ...
        fprintf(logFid, 'Resistances Mixer 1 = [Sm, Dy, Eu, mix] [%s]\n', num2str([mixData.M1.R1, mixData.M1.R2, mixData.M1.R4, mixData.M1.Rmix], '%4.2f  '));
        fprintf(logFid, 'Resistances Mix2r 2 = [Sm, Dy, Eu, mix] [%s]\n', num2str([mixData.M2.R1, mixData.M2.R2, mixData.M2.R4, mixData.M2.Rmix], '%4.2f  '));
        ...
        fprintf(logFid, 'Lanthanide MFCS Channels: [Sm, Dy, Eu]  [%s]\n', num2str(lanthanideMfcsCh, '%i,  '));
        str = lanthanideValves{1};
        for ii = 2:length(lanthanideValves)
            str = [str ',  ' lanthanideValves{ii}];
        end
        fprintf(logFid, 'Lanthanide Valves: [Sm, Dy, Eu]  [%s]\n', str);
        fprintf(logFid, 'Mixer 1\n');
        fprintf(logFid, 'Code#\tFlow_Sm\tFlow_Dy\tFlow_Eu\tP_Sm\tP_Dy\tP_Eu\n');
        doLog = true;
    else
        doLog = false;
    end
    ...
    for ii = 1:length(Sm)
        for jj = 1:length(Dy)
            %for kk = 1:length(Tm)
                if (Sm(ii) + Dy(jj)) <= 1
                    numCodes = numCodes + 1;
                    flows(:, numCodes) = [Sm(ii); Dy(jj); 1 - (Sm(ii) + Dy(jj)); 1];
                end
            %end
        end
    end
...
%     disp(mixData.M2.inputPressures);
    ...
    mixData.M2.inputPressures = mixData.M1.inputPressures(:,2:2:end);
    mixData.M1.inputPressures = mixData.M1.inputPressures(:,1:2:end);
    
    mixData.M1.codes = 1:2:numCodes;
    mixData.M2.codes = 2:2:numCodes;
    
    if doLog
        for ii = 1:size(mixData.M1.inputPressures,2)
        ...
        for ii = 1:size(mixData.M2.inputPressures,2)
...
%             uiwait(msgbox(['Set push pressure to ' num2str(pushWaterPressure) ', oil pressure to ' num2str(oilPressure) ' and flush pressure to ' num2str(flushPressure)]));
            ...
            for ii = 1:size(mixData.M1.inputPressures,2)
                scriptCheck4Stop;
                codeStr.M1 = ['Code # ' num2str(mixData.M1.codes(ii)) ' = [' num2str(flows(1:4, mixData.M1.codes(ii))', '%4.2f  ') ']'];
                ...
                chip.openValves({'Mx1Out', 'Mx1In'}); pause(0.3)
                chip.openValve('Water'); pause(0.3)
                chip.openValve('MxWaste'); pause(0.3)
                scr.wait(0, mixerCleanTime, [codeStr.M1 10 13 'Cleaning mixer 1'], true); 
                chip.closeValve('Water'); pause(0.3)
                ...
                if ii>1
                    scr.wait(0, dropPushStabilizationTime, [codeStr.M2 10 13 'Stabilizing Mixer 2 drop push pressure'], true);
                    % Open t-junction for mixer 2
                    chip.openValve('Drops2'); pause(0.3)
                    ...
                    scope.gui.updateGUI(1);
                    scope.gui.enableLiveMode(1);
                    chip.closeValve('Drops2'); pause(0.3)
                    chip.closeValve('Push2'); pause(0.3)
                else
                    scr.wait(mixingTime(1), mixingTime(2), [codeStr.M1 10 13 'Mixing lanthanides Mixer 1'], true);
                end
                
                chip.closeValves(lanthanideValves); pause(0.3)
%                 chip.closeValve('MxWaste');
                chip.closeValves({'Mx1Out', 'Mx1In'}); pause(0.3)
                chip.openValve(pushWaterResistor); pause(0.3)
                chip.openValve('Push1'); pause(0.3)
                
                
                % Flushing mixer 2 with water before making a code
                if ii<= size(mixData.M2.inputPressures,2)
                    codeStr.M2 = ['Code # ' num2str(mixData.M2.codes(ii)) ' = [' num2str(flows(1:4, mixData.M2.codes(ii))', '%4.2f  ') ']'];
                
                    chip.openValves({'Mx2Out', 'Mx2In'}); pause(0.3)
                    chip.openValve('Water');  pause(0.3)
                    chip.openValve('MxWaste'); pause(0.3)
                    scr.wait(0, mixerCleanTime, [codeStr.M2 10 13 'Cleaning mixer 2'], true);
                    chip.closeValve('Water'); pause(0.3)
                
                    mfcs.setChannelsPressure(lanthanideMfcsCh, mixData.M2.inputPressures(2:5, ii));
                    % Opening only the valves with flow rate > 0
                    % Input pressure > pressure at mixer 2 entrance
                
                    if mixData.M2.inputPressures(1,ii) < (mixData.M2.inputPressures(2, ii) - 0.05) %Sm
                        chip.openValve(lanthanideValves{1});
                    end
                    if mixData.M2.inputPressures(1,ii) < (mixData.M2.inputPressures(3, ii) - 0.05) %Dy
                        chip.openValve(lanthanideValves{2});
                    end
                    %if mixData.M2.inputPressures(1,ii) < (mixData.M2.inputPressures(4, ii) - 0.05) %Tm
                    %    chip.openValve(lanthanideValves{3});
                    %end
                    if mixData.M2.inputPressures(1,ii) < (mixData.M2.inputPressures(5,ii) - 0.05) %Eu
                        chip.openValve(lanthanideValves{3});
                    end
                    ...
                    chip.openValve('Drops1'); pause(0.3)
                    ...
                    scope.gui.updateGUI(1);
                    scope.gui.enableLiveMode(1);
                    chip.closeValve('Drops1'); pause(0.3)
                    chip.closeValve('Push1'); pause(0.3)
                
                
                    chip.closeValves(lanthanideValves); pause(0.3)
                    chip.closeValves({'Mx2Out', 'Mx2In'}); pause(0.3)
                    chip.openValve(pushWaterResistor); pause(0.3)
                    chip.openValve('Push2'); pause(0.3)
                    
                    if ii == size(mixData.M1.inputPressures,2)
                        scr.wait(0, dropPushStabilizationTime, [codeStr.M2 10 13 'Stabilizing Mixer 2 drop push pressure'], true);
                        % Open t-junction for mixer 2
                        chip.openValve('Drops2'); pause(0.3)
                        scr.wait(0, dropStabilizationTime, [codeStr.M2 10 13 'Waiting for drop stabilization Mixer 2'], true);
                        ...
                        scr.wait(polymerTime(1), polymerTime(2), [codeStr.M2 10 13 'Making Beads Mixer 2'], true);
                        scope.gui.enableLiveMode(0);
                        scope.mmc.setShutterOpen(0);
                        scope.mmc.setConfig('Channel', 'BF');
                        scope.gui.updateGUI(1);
