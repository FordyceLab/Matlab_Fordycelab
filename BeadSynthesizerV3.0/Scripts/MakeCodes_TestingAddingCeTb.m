% Script to generate 50 code set with Tm = [0.3]
% 10 levels of Sm and Dy

%Eu is in the C6 input on Chip 3 due to blockage at C1.

% Also optimized the herringbone fill (1:45) and empty (~4:00) times, but won't adjust the empty with push water time. 

% P. Fordyce and B. Baxter 07/02/14
% Attempting to add additional logging about actual MFCS pressures

% Path and name of file to write log to
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140703-01.txt');
mfcsLogFile = fullfile(uigetdir(), filesep, 'PressureLog_20140703-01.txt');

% Chip serial code
chipSerial = 'BS3.0A-PMF-06.12.14-ch03';

% Number of lanthanides including reference color
colors = {'Sm', 'Dy', 'Tm', 'CeTb','Eu'};
numColors = numel(colors);

% Lanthanides levels wanted
Sm = [0, 0.0403, 0.0892, 0.1486, 0.2207, 0.3082, 0.4146, 0.5437, 0.7005, 0.8908];
Dy = [0, 0.0326, 0.0732, 0.1237, 0.1865, 0.2647, 0.3619, 0.4829, 0.6334, 0.8207];
Tm = [0.3];
CeTb = [0]

allLevels = {colors{1}, Sm; colors{2}, Dy; colors{3}, Tm; colors{4}, CeTb};
...
% [Sm, Dy, Tm, CeTb, Eu]
lanthanideMfcsCh = [6, 7, 4, 8, 5];

% Chip input valve names used for the lanthanides
% [Sm, Dy, Tm, CeTb, Eu]
%lanthanideValves = {'C2'; 'C3'; 'C4'; 'C1'};
lanthanideValves = {'C2'; 'C3'; 'C4'; 'C7'; 'C6'}; %JUST FOR THIS CHIP #3!!
...
% Mixer 1
mixData.M1.C1 = 1.0281; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.0343; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 1.210; %RTm/REu = WidthEu/WidthTm
mixData.M1.C4 = 1.0; %RCeTb/REu = WidthEu/WidthCeTb
mixData.M1.Cmix = 1/61.50; %Rmix/REu = Pmix/(PEu - Pmix)
% Mixer 2
...
mixData.M2.C4 = 1.0; %RCeTb/REu = WidthEu/WidthCeTb
mixData.M2.Cmix = 1/49.00; %Rmix/REu = Pmix/(PEu - Pmix)


% Input and mixer resistances
% Mixer 1
mixData.M1.R5 = maxPressure; %Eu
mixData.M1.R1 = mixData.M1.C1*mixData.M1.R5; %Sm
mixData.M1.R2 = mixData.M1.C2*mixData.M1.R5; %Dy
mixData.M1.R3 = mixData.M1.C3*mixData.M1.R5; %Tm
mixData.M1.R4 = mixData.M1.C4*mixData.M1.R5; %CeTb
mixData.M1.Rmix = mixData.M1.Cmix*mixData.M1.R5;
mixData.M1.Resistors = {mixData.M1.R1, mixData.M1.R2, mixData.M1.R3, mixData.M1.R4, mixData.M1.R5, mixData.M1.Rmix};
% Mixer 2
mixData.M2.R5 = maxPressure; %Eu
mixData.M2.R1 = mixData.M2.C1*mixData.M2.R5; %Sm
mixData.M2.R2 = mixData.M2.C2*mixData.M2.R5; %Dy
mixData.M2.R3 = mixData.M2.C3*mixData.M2.R5; %Tm
mixData.M2.R4 = mixData.M2.C4*mixData.M2.R5; %CeTb
mixData.M2.Rmix = mixData.M2.Cmix*mixData.M2.R5;
mixData.M2.Resistors = {mixData.M2.R1, mixData.M2.R2, mixData.M2.R3, mixData.M2.R4, mixData.M2.R5, mixData.M2.Rmix};
...
if ~scr.stopSignal()
    if ~isempty(logFile)
        ...
        fprintf(logFid, 'Resistances Mixer 1 = [Sm, Dy, Tm, CeTb, Eu, mix] [%s]\n', num2str([mixData.M1.R1, mixData.M1.R2, mixData.M1.R3, mixData.M1.R4, mixData.M1.R5, mixData.M1.Rmix], '%4.2f  '));
        fprintf(logFid, 'Resistances Mix2r 2 = [Sm, Dy, Tm, CeTb, Eu, mix] [%s]\n', num2str([mixData.M2.R1, mixData.M2.R2, mixData.M2.R3, mixData.M2.R4, mixData.M2.R5, mixData.M2.Rmix], '%4.2f  '));
        ...
        fprintf(logFid, 'Lanthanide MFCS Channels: [Sm, Dy, Tm, CeTb, Eu]  [%s]\n', num2str(lanthanideMfcsCh, '%i,  '));
        str = lanthanideValves{1};
        for ii = 2:length(lanthanideValves)
            str = [str ',  ' lanthanideValves{ii}];
        end
        fprintf(logFid, 'Lanthanide Valves: [Sm, Dy, Tm, CeTb, Eu]  [%s]\n', str);
        fprintf(logFid, 'Mixer 1\n');
        fprintf(logFid, 'Code#\tFlow_Sm\tFlow_Dy\tFlow_Tm\tFlow_CeTb\tFlow_Eu\tP_Sm\tP_Dy\tP_Tm\tP_CeTb\tP_Eu\n');
        doLog = true;
    else
        doLog = false;
    end
    ...
    % flows(:, 4) = CeriumTerbium-Eu flow
    % flows(:, 5) = Ballast (Eu only) flow
    % flows(:, 6) = Total flow = 1
    numCodes = 0;
    for ii = 1:length(Sm)
        for jj = 1:length(Dy)
            for kk = 1:length(Tm)
                for ll = 1:length(CeTb)
                    if (Sm(ii) + Dy(jj) + Tm(kk) + CeTb(ll)) <= 1
                        numCodes = numCodes + 1;
                        flows(:, numCodes) = [Sm(ii); Dy(jj); Tm(kk); CeTb(ll);1 - (Sm(ii) + Dy(jj) + Tm(kk)+CeTb(ll)); 1];
                    end
                end
            end
        end
    ...
    % inputPressures(5, n) = P_CeTb
    % inputPressures(6, n) = P_Eu
...
%                 uiwait(msgbox(['Set mfcs channels [' num2str(lanthanideMfcsCh) '] to ' num2str(inputPressures(2:5, ii)'.*68.94) 'mBar']));
                ...
                pause(0.1)
                if mixData.M1.inputPressures(1,ii) < (mixData.M1.inputPressures(5, ii) - 0.05) %CeTb
                    chip.openValve(lanthanideValves{4});
                end
                pause(0.1)
                if mixData.M1.inputPressures(1,ii) < (mixData.M1.inputPressures(6,ii) - 0.05) %Eu
                    chip.openValve(lanthanideValves{5});
                end
%                 if ii>1
