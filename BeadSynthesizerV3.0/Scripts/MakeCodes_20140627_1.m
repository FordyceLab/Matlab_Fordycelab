% Script to generate 192 (os so, I forgot) code set with Eu, Sm, Dy, and Tm
% Also optimized the herringbone fill (1:45) and empty (~4:00) times, but won't adjust the empty with push water time. 
% P. Fordyce and B. Baxter 06/26/14
% Attempting to add additional logging about actual MFCS pressures

% Path and name of file to write log to
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20140626-01.txt');
mfcsLogFile = fullfile(uigetdir(), filesep, 'PressureLog_20140626-01.txt');
...
% Mixer 1
mixData.M1.C1 = 1.0633; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 1.080; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 1.1429; %RTm/REu = WidthEu/WidthTm
