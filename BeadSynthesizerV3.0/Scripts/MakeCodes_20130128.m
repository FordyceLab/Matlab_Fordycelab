...
logFile = fullfile(uigetdir(), filesep, 'SynthesisLog_20130128-01.txt');

% Chip serial code
chipSerial = 'BS3.0-CAD-01.18.13-chxx';
...
pushWaterPressure = 3.98;
% Push water resistor
pushWaterResistor = 'Rp2';
...
% [Sm, Dy, Tm, Eu]
lanthanideMfcsCh = [7, 5, 8, 4];

% Chip input valve names used for the lanthanides
% [Sm, Dy, Tm, Eu]
lanthanideValves = {'C7'; 'C2'; 'C5'; 'C1'};
...
% Mixer 1
mixData.M1.C1 = 0.6987; %RSm/REu = WidthEu/WidthSm
mixData.M1.C2 = 0.594; %RDy/REu = WidthEu/WidthDy
mixData.M1.C3 = 0.594; %RTm/REu = WidthEu/WidthTm
mixData.M1.Cmix = 1/34; %Rmix/REu = Pmix/(PEu - Pmix)
% Mixer 2
mixData.M2.C1 = 0.7514; %RSm/REu = WidthEu/WidthSm
mixData.M2.C2 = 0.6664; %RDy/REu = WidthEu/WidthDy
mixData.M2.C3 = 0.6664; %RTm/REu = WidthEu/WidthTm
mixData.M2.Cmix = 1/27; %Rmix/REu = Pmix/(PEu - Pmix)
...
if ~scr.stopSignal()
    if ~isempty(logFile)
        ...
        fprintf(logFid, 'Polymerization excitation = %s\n', polymerExcitation);
...
%                 uiwait(msgbox(['Set mfcs channels [' num2str(lanthanideMfcsCh) '] to ' num2str(inputPressures(2:5, ii)'.*68.94) 'mBar']));
                ...
                chip.openValves({'Mix1Out', 'Mix1In'})
                ...
                if ii>1
                    ...
                    if ~scr.stopSignal()
                        ...
                        scope.mmc.setConfig('Excitation', polymerExcitation);
                        scope.gui.updateGUI(1);
...
%                 chip.closeValve('Water');
%                 chip.openValve('Ro0');
                scr.wait(2, 0, 'Final Oil Flushing', true);
                chip.closeValve('Ro0');
%                 chip.closeValve('Ro2');
