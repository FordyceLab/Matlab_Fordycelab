...
spectra=[dy_norm,er_norm,eu_norm,sm_norm,tm_norm];
nspec=size(spectra,2);
ntests=20000;
errorlevels=logspace(-2,-0.602,10);
...
for p=1:size(errorlevels,2)
    ...
    for n=1:ntests
        ...
        for s=1:nspec
            ...
            test_spectrum=test_spectrum + temp_spectrum + temp_spectrum .* errorlev .* randn(size(temp_spectrum));
        end
