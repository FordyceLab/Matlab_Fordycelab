...
if isfield(OptionZ,'Duration') %space out view angles
    ...
    %space durations so that each step moves a constant number of degrees
    delta_angle = abs(sum(diff(ViewZ),2));
    nFrames = round(delta_angle*(temp_n/sum(delta_angle)));
        ViewZ_new=zeros(sum(nFrames),2);
    % space view angles, if needed
    fstart = 1;
    for s = 1:numel(nFrames)
        temp = [linspace(ViewZ(s,1), ViewZ(s+1,1), nFrames(s)); linspace(ViewZ(s,2), ViewZ(s+1,2), nFrames(s))]';
        ViewZ_new(fstart:fstart+nFrames(s)-1, :) = temp;
        fstart = fstart+nFrames(s);
    end
 
%Old Angle Spacing Code
